...
val = video.split('.')[0]
csv_file = f'{val}.csv'
final_video = f'{val}_final_normalised.mp4'
        ...
        def process_frame(get_frame, t):
            ...
            if results.pose_landmarks:
                ...
                if prev_landmarks is not None:
                    distance = calculate_rms_distances(prev_landmarks, curr_landmarks)
                    
                    distances_list.append((t, distance))

                    
                    window_start_time = max(0, t - 5)
                    window_distances = [d for time, d in distances_list if time >= window_start_time]

                    if distance == max(window_distances):
                        peak_times.append(t)
                        # Apply effect
