...
def detect_movement(current_landmarks, prev_landmarks):
    movement_threshold = 0.45  # Threshold to consider as movement
...
def main():
    ...
    out_video = 'illuminati_final2.mp4'
    scale_percent = 50
    ...
    def process_clip(get_frame, t):
        ...
        # Apply pan effect based on timestamp
        for start, end in effect_timestamps:
            if start <= timestamp <= end:
                effect = 'left' if start % 2 == 0 else 'right'
                image = zoom_and_pan(image, effect)
                break

        return image

    # Define the effect timestamps
    effect_timestamps = [(4, 5), (11, 12)]
